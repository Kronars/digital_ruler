...
# window config
framename: str      = "Remora - Camera Control"
framename_main: str = "Remora - Live Video stream"
...
while True:
    ...
    cv2.normalize(frame_main, frame_main, 0, 255, norm_type=cv2.NORM_MINMAX)
    ...
    # -------------------------------
        ...
        if mouse_mark:
            ...
            draw.add_text(frame0, f'{xlen:.2f}', x1-((x1-x2)/2),
                          min(y1, y2)-8, center=True, color='red')
            draw.add_text(
                frame0, f'Area: {carea:.2f}', x3, y3+8, center=True, top=True, color='red')
            if alen:
                draw.add_text(
    ...
    cv2.imshow(framename_main, frame_main)
