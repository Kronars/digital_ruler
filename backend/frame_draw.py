...
class DRAW:
    ...
    def circle(self, frame, x1, y1, x2, y2, r, weight=1, color='green', filled=False):
        if filled:
            weight = -1
        cv2.circle(frame, (int(x1), int(y1)), (int(x2), int(y2)),
    ...
    def crosshairs(self, frame, offset = 10, weight = 1, color = 'green', invert = False):
        ...
        if invert:
            self.line(frame, 0, ycenter, xcenter -
                      offset, ycenter, weight, color)
            self.line(frame, xcenter+offset, ycenter,
                      self.width, ycenter, weight, color)
            self.line(frame, xcenter, 0, xcenter,
                      ycenter-offset, weight, color)
            self.line(frame, xcenter, ycenter+offset,
                      xcenter, self.height, weight, color)
        else:
            self.line(frame, xcenter-offset, ycenter,
                      xcenter+offset, ycenter, weight, color)
            self.line(frame, xcenter, ycenter-offset,
                      xcenter, ycenter+offset, weight, color)
